...
const lastPlayTimeByUser = new Map<number, number>();
...
function updateCardInCache(cardId: number, updates: any) {
  if (cachedCardTemplates) {
    const index = cachedCardTemplates.findIndex(c => c.id === cardId);
    if (index !== -1) {
      cachedCardTemplates[index] = { ...cachedCardTemplates[index], ...updates };
    }
  }
}
...
const FALLBACK_STATS: Record<string, RarityStats> = {
  COMMON: { hp: { min: 30, max: 100 }, def: { min: 0, max: 20 }, atk: { min: 1, max: 25 }, price: { min: 0.01, max: 0.30 } },
  UNCOMMON: { hp: { min: 110, max: 180 }, def: { min: 15, max: 35 }, atk: { min: 20, max: 40 }, price: { min: 0.70, max: 1.10 } },
  SPARSE: { hp: { min: 150, max: 250 }, def: { min: 20, max: 45 }, atk: { min: 35, max: 60 }, price: { min: 2.00, max: 3.00 } },
  RARE: { hp: { min: 300, max: 400 }, def: { min: 30, max: 60 }, atk: { min: 40, max: 90 }, price: { min: 6.00, max: 9.00 } },
  UBER_RARE: { hp: { min: 300, max: 650 }, def: { min: 20, max: 100 }, atk: { min: 30, max: 125 }, price: { min: 7.00, max: 14.00 } },
  MYTHICAL: { hp: { min: 550, max: 950 }, def: { min: 40, max: 120 }, atk: { min: 60, max: 150 }, price: { min: 20.00, max: 35.00 } },
  LEGENDARY: { hp: { min: 750, max: 1250 }, def: { min: 50, max: 150 }, atk: { min: 100, max: 200 }, price: { min: 50.00, max: 100.00 } },
  SPECIAL: { hp: { min: 1000, max: 2000 }, def: { min: 200, max: 300 }, atk: { min: 250, max: 350 }, price: { min: 250.00, max: 1000.00 } }
};
...
function weightedRandom(min: number, max: number, skew: number = 1): number {
  ...
  num = num / 4.0;
  if (skew !== 1) num = Math.pow((num + 1) / 2, skew) * 2 - 1;
...
function generateFallbackStats(rarity: string, existingStats?: { hp?: number; atk?: number; def?: number; price?: number }) {
  const ranges = FALLBACK_STATS[rarity] || FALLBACK_STATS.COMMON;
  let price = existingStats?.price ?? roundCurrency(weightedRandom(ranges.price.min, ranges.price.max, 1));
...
async function ensureCardStats(cardTemplate: any): Promise<any> {
  ...
  if (needsFallback) {
    ...
    updateCardInCache(cardTemplate.id, fallbackStats);
    return { ...cardTemplate, ...fallbackStats };
  }
  return cardTemplate;
}
  ...
  switch (rewardType) {
    ...
    case Reward.CARD: {
      ...
      for (let i = 0; i < qualityWeights.length; i++) {
        ...
        if (randomNum <= cumulative) { selectedQuality = qualities[i]; break; }
      }
      ...
      for (let i = 0; i < enhancementWeights.length; i++) {
        ...
        if (enhancementRandom <= enhancementCumulative) { selectedEnhancement = enhancements[i]; break; }
      }
    ...
    case Reward.PACK: {
      ...
      if (packExists) {
        await prisma.userInventory.create({
          data: { user_id: userId, item_type: 'PACK', reference_id: packId, quantity: 1 }
        });
      } else {
    ...
    case Reward.COSMETIC:
      await prisma.userInventory.create({
        data: { user_id: userId, item_type: 'ITEM', reference_id: parseInt(rewardValue), quantity: 1 }
      });
      break;
    case Reward.STATUS:
      await prisma.user.update({
...
async function checkAndUpdateAchievements(userId: number, triggerCondition: Condition, cardData?: any) {
  const checkKey = `${userId}-${triggerCondition}`;
  if (activeChecks.has(checkKey)) return;
  activeChecks.add(checkKey);
  ...
  try {
    ...
    let itemCountCache: number | null = null;
    
    for (const ach of achievements) {
      let currentValue: number = 0;
      switch (ach.condition) {
        case Condition.WINS: currentValue = user.userStats.wins; break;
        case Condition.LOSSES: currentValue = user.userStats.losses; break;
        case Condition.CURRENCY: currentValue = user.currency; break;
        case Condition.CARDS_COLLECTED: currentValue = user.userStats.unique_cards; break;
        case Condition.PACKS_OPENED: currentValue = user.userStats.total_pulls; break;
        case Condition.TRADES_COMPLETED: currentValue = user.userStats.trades_completed; break;
        case Condition.PLAY_TIME: currentValue = user.userStats.total_play_minutes; break;
        case Condition.CARDS_SOLD: currentValue = user.userStats.total_cards_sold; break;
        case Condition.CURRENCY_SPENT: currentValue = user.userStats.total_currency_spent; break;
        case Condition.CURRENCY_GAINED: currentValue = user.userStats.total_currency_gained; break;
        case Condition.PURCHASES_MADE: currentValue = user.userStats.purchases_made; break;
        case Condition.CARD_HEALTH: currentValue = cardData?.base_hp ?? 0; break;
        case Condition.CARD_STRENGTH: currentValue = cardData?.base_atk ?? 0; break;
        case Condition.CARD_DEFENCE: currentValue = cardData?.base_def ?? 0; break;
        case Condition.CARD_NAME_LENGTH: currentValue = cardData?.name?.length ?? 0; break;
        case Condition.CARD_NAME_WORDS: currentValue = cardData?.name?.split(/\s+/).filter((w: string) => w.length > 0).length ?? 0; break;
        case Condition.CARD_RARITY: currentValue = (cardData?.rarity === ach.value_string) ? 1 : 0; break;
        case Condition.INVENTORY_ITEMS:
          if (itemCountCache === null) {
            ...
            itemCountCache = itemCount._sum.quantity || 0;
          }
          currentValue = itemCountCache;
          break;
    ...
    const toUpdate: { id: number; progress: number }[] = [];
    const toCreate: { user_id: number; achievement_id: number; progress: number }[] = [];
    ...
    for (const ach of achievements) {
      ...
      if (existing) {
        if (existing.progress !== progress) toUpdate.push({ id: existing.id, progress });
      } else {
        toCreate.push({ user_id: userId, achievement_id: ach.id, progress });
      }
    }
    
    if (toUpdate.length > 0) {
      await Promise.all(toUpdate.map(u => prisma.userAchievement.update({ where: { id: u.id }, data: { progress: u.progress } })));
    }
    if (toCreate.length > 0) {
      await prisma.userAchievement.createMany({ data: toCreate });
    }

    const newlyCompleted: { ach: any; rewardType: Reward; rewardValue: string }[] = [];
    ...
    for (const ach of achievements) {
      ...
      switch (ach.comparator) {
        case Comparator.MORE_THAN: isComplete = currentValue > targetValue; break;
        case Comparator.LESS_THAN: isComplete = currentValue < targetValue; break;
        case Comparator.EQUAL: isComplete = currentValue === targetValue; break;
        case Comparator.MORE_OR_EQUAL: isComplete = currentValue >= targetValue; break;
        case Comparator.LESS_OR_EQUAL: isComplete = currentValue <= targetValue; break;
      }
      
      const existing = existingMap.get(ach.id);
      if (isComplete && (!existing || !existing.completed_at)) {
        newlyCompleted.push({ ach, rewardType: ach.reward_type, rewardValue: ach.reward_value });
      }
    }
    
    // Process all newly completed achievements
    for (const { ach, rewardType, rewardValue } of newlyCompleted) {
      await grantReward(userId, rewardType, rewardValue);
      ...
      if (userData) {
        ...
        sendAchievementNotification(
          ...
          updatedUser?.currency ?? 0,
          updatedAchievements
        );
      }
    }
    
    // Only check ACHIEVEMENT_COUNT if we actually completed something
...
function sendAchievementNotification(userId: string, achievementName: string, reward: string, currency?: number, achievements?: any[]) {
...
async function generatePackCards(pack: any, userId: number) {
  ...
  if (pack.included_types && Array.isArray(pack.included_types)) {
    availableCards = availableCards.filter((card: any) => pack.included_types.includes(card.type));
  }
  if (pack.excluded_series && Array.isArray(pack.excluded_series)) {
    availableCards = availableCards.filter((card: any) => !pack.excluded_series.includes(card.series));
  }
  ...
  for (let i = 0; i < remainingCards; i++) {
    ...
    let selectedCard = cardsOfRarity.length > 0
    ...
    for (const q of qualities) {
      ...
      if (qualityRandom <= qualityCumulative) { quality = q.name; break; }
    }
    ...
    for (const e of enhancements) {
      ...
      if (enhancementRandom <= enhancementCumulative) { enhancement = e.name; break; }
    }
      ...
      if (isComplete) {
        ...
        if (!existing || !existing.completed_at) {
          ...
          await prisma.userAchievement.upsert({
            where: { user_id_achievement_id: { user_id: userId, achievement_id: ach.id } },
            update: { completed_at: new Date(), progress: 100 },
            create: { user_id: userId, achievement_id: ach.id, progress: 100, completed_at: new Date() }
          });
        }
      }
    }
...
app.post('/api/user-login', async (req, res) => {
  ...
  try {
    ...
    const hasBadge = await prisma.userInventory.findFirst({
      where: { user_id: dbUser.id, item_type: 'ITEM', reference_id: 5 }
    });
    if (!hasBadge) {
      await prisma.userInventory.create({
        data: {
...
app.post('/api/user/currency', async (req, res) => {
  ...
  if (amount > 0) await updateCurrency(user.id, amount, 'gain');
  else if (amount < 0) await updateCurrency(user.id, Math.abs(amount), 'spend');
...
app.get('/api/user/inventory', async (req, res) => {
  ...
  const items = await prisma.userInventory.findMany({ where: { user_id: user.id } });
  ...
  const itemsWithDetails = items.map(inv => {
    ...
    if (inv.item_type === 'ITEM') {
      ...
      if (details) { canSell = details.can_sell || false; if (canSell) sellPrice = details.price * 0.5; }
    } else if (inv.item_type === 'PACK') {
      details = packMap.get(inv.reference_id);
      if (details) { canSell = true; sellPrice = details.price * 0.8; }
    }
    return { ...inv, name: details?.name || 'Unknown', image_url: details?.image_url, description: details?.description || '', sell_price: sellPrice, can_sell: canSell };
  });
  ...
  try {
    ...
    const userCards = await prisma.userCards.findMany({
      ...
      select: {
        ...
        cardTemplate: {
          select: {
            id: true, name: true, image_url: true, rarity: true,
            description: true, base_price: true, base_hp: true,
            base_atk: true, base_def: true, series: true, type: true
          }
        }
      }
    });
...
app.post('/api/user/reset-collection', async (req, res) => {
  ...
  try {
    ...
    await prisma.userStats.update({
      where: { user_id: user.id },
      data: { unique_cards: 0, total_cards_sold: 0 }
    });
    res.json({ success: true });
  } catch (error) {
    console.error('Reset error:', error);
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to reset progress' });
  }
});
...
app.post('/api/user/clear-all-data', async (req, res) => {
  ...
  try {
    ...
    await prisma.user.update({ where: { id: user.id }, data: { currency: 0 } });
    await prisma.userStats.create({
      data: {
        user_id: user.id,
        total_pulls: 0, unique_cards: 0, wins: 0, losses: 0,
        trades_completed: 0, purchases_made: 0, total_play_minutes: 0,
        total_cards_sold: 0, consecutive_wins: 0, highest_win_streak: 0,
        consecutive_losses: 0, highest_lose_streak: 0, battle_rating: 1000
      }
    });
    res.json({ success: true });
  } catch (error) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const card = await tx.userCards.findFirst({
        where: { id: cardId, user: { clerkId: auth.userId } },
      ...
      await Promise.all([
        ...
        tx.userStats.update({
          ...
          data: { total_currency_gained: { increment: sellPrice }, total_cards_sold: { increment: 1 } }
        }),
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to sell card' });
  }
});
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const cards = await tx.userCards.findMany({
        where: { id: { in: cardIds }, user: { clerkId: auth.userId } },
        include: { cardTemplate: true, user: { select: { id: true, currency: true } } }
      });
      ...
      await Promise.all([
        ...
        tx.userStats.update({
          ...
          data: { total_currency_gained: { increment: totalSellPrice }, total_cards_sold: { increment: cards.length } }
        }),
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to sell cards' });
  }
});
...
app.get('/api/packs', async (_req, res) => {
  try {
    const packs = await prisma.pack.findMany({
      ...
      select: { id: true, name: true, description: true, price: true, image_url: true, cards_count: true }
    });
    res.json({ packs });
  } catch (error) {
...
app.post('/api/gacha/pack', async (req, res) => {
  ...
  const inventoryPack = await prisma.userInventory.findFirst({
    where: { user_id: user.id, item_type: 'PACK', reference_id: packId, quantity: { gt: 0 } },
  ...
  const [cards, _updateResult] = await Promise.all([
    ...
    (async () => {
      if (inventoryPack) {
        ...
        if (user.currency < pack.price) throw new Error('Insufficient currency');
...
app.get('/api/shop/items', async (req, res) => {
  ...
  if (!auth.userId) return res.status(401).json({ error: 'Unauthorized' });
  try {
    const dbItems = await prisma.shopItem.findMany({ where: { is_available: true } });
    const regularItems = dbItems.filter(item => item.item_type !== 'CARD_SLOT' && item.item_type !== 'MYTHICAL_CARD');
    ...
    for (const rarity of rarities) {
      ...
      if (cards.length > 0) {
        ...
        cardSlots.push({
          id: ensuredCard.id, name: ensuredCard.name,
          description: ensuredCard.description || `${rarity} card`,
          item_type: 'CARD_SLOT', reference_id: ensuredCard.id,
          price: (ensuredCard.base_price || 1) * 2,
          image_url: ensuredCard.image_url, rarity,
          is_available: true, base_hp: ensuredCard.base_hp,
          base_atk: ensuredCard.base_atk, base_def: ensuredCard.base_def,
          base_price: ensuredCard.base_price, series: ensuredCard.series,
          type: ensuredCard.type
        });
      }
    }
    
    const mythicalCards = await prisma.cardTemplates.findMany({ where: { rarity: 'MYTHICAL' as Rarity } });
    ...
    if (mythicalCards.length > 0) {
      ...
      mythicalSlot = {
        id: ensuredMythical.id, name: ensuredMythical.name,
        description: ensuredMythical.description || 'Mythical card',
        item_type: 'MYTHICAL_CARD', reference_id: ensuredMythical.id,
        price: (ensuredMythical.base_price || 20) * 2,
        image_url: ensuredMythical.image_url, rarity: 'MYTHICAL',
        is_available: true, base_hp: ensuredMythical.base_hp,
        base_atk: ensuredMythical.base_atk, base_def: ensuredMythical.base_def,
        base_price: ensuredMythical.base_price, series: ensuredMythical.series,
        type: ensuredMythical.type
      };
    }
    
    const allItems = [...regularItems, ...cardSlots];
    if (mythicalSlot) allItems.push(mythicalSlot);
...
app.post('/api/shop/purchase', async (req, res) => {
  ...
  try {
    ...
    let reward = null;
    const cardTemplate = await prisma.cardTemplates.findUnique({ where: { id: itemId } });
    if (cardTemplate && quality && enhancement) {
      ...
      const shopItem = await prisma.shopItem.findUnique({ where: { id: itemId, is_available: true } });
      if (!shopItem) return res.status(404).json({ error: 'Item not found' });
      switch (shopItem.item_type) {
        case 'ONE_TIME_PACK':
        case 'MULTI_BUY_PACK': {
          const pack = await prisma.pack.findUnique({ where: { id: shopItem.reference_id } });
          if (!pack) return res.status(404).json({ error: 'Pack not found' });
          await prisma.userInventory.create({
            data: { user_id: user.id, item_type: 'PACK', reference_id: shopItem.reference_id, quantity: 1 }
          });
          reward = { type: 'pack', pack };
          break;
        }
        case 'ITEM_SLOT': {
          const item = await prisma.item.findUnique({ where: { id: shopItem.reference_id } });
          if (!item) return res.status(404).json({ error: 'Item not found' });
          await prisma.userInventory.create({
            data: { user_id: user.id, item_type: 'ITEM', reference_id: shopItem.reference_id, quantity: 1 }
          });
    ...
    if (!reward) return res.status(500).json({ error: 'Failed to generate reward' });
    
    await prisma.user.update({ where: { id: user.id }, data: { currency: { decrement: price } } });
    await prisma.userStats.update({
      where: { user_id: user.id },
      data: { purchases_made: { increment: 1 }, total_currency_spent: { increment: price } }
    });
    if (isOneTime) {
      await prisma.shopPurchase.create({
        data: { user_id: user.id, slot_id: slotId, price_paid: price, item_id: itemId }
      });
    }
...
app.post('/api/inventory/sell', async (req, res) => {
  ...
  try {
    ...
    const inventoryItem = await prisma.userInventory.findFirst({
      where: { id: inventoryId, user_id: user.id }
    });
    ...
    if (inventoryItem.item_type === 'PACK') {
      const pack = await prisma.pack.findUnique({ where: { id: inventoryItem.reference_id } });
      if (!pack) return res.status(404).json({ error: 'Pack not found' });
      sellPrice = pack.price * 0.8;
      itemName = pack.name;
    } else if (inventoryItem.item_type === 'ITEM') {
      const item = await prisma.item.findUnique({ where: { id: inventoryItem.reference_id } });
      if (!item) return res.status(404).json({ error: 'Item not found' });
      if (!item.can_sell) return res.status(400).json({ error: 'This item cannot be sold' });
...
app.post('/api/user/check-completions-batch', async (req, res) => {
  const auth = getAuth(req);
  if (!auth.userId) return res.status(401).json({ error: 'Unauthorized' });
  const { cardTemplateIds } = req.body;
  
  if (!cardTemplateIds || !Array.isArray(cardTemplateIds)) {
    return res.status(400).json({ error: 'cardTemplateIds array required' });
  }
  ...
  // Get ALL user's cards in one query
  const userCards = await prisma.userCards.findMany({
    where: { user_id: user.id }
  });
  
  // Get existing completions
  const existingCompletions = await prisma.userCardCompletion.findMany({
    where: {
      user_id: user.id,
      card_template_id: { in: cardTemplateIds }
    }
  });
  const completedSet = new Set(existingCompletions.map(c => c.card_template_id));
  
  const allQualities = ['TARNISHED', 'POOR', 'REGULAR', 'GOOD', 'CRISP'];
  const allEnhancements = ['BASIC', 'FOILED', 'SHINY', 'SIGNED'];
  
  const toReward: number[] = [];
  
  for (const templateId of cardTemplateIds) {
    if (completedSet.has(templateId)) continue;
    
    const cardsForTemplate = userCards.filter(c => c.card_template_id === templateId);
    let hasAll = true;
    
    for (const quality of allQualities) {
      for (const enhancement of allEnhancements) {
        const found = cardsForTemplate.some(c => c.quality === quality && c.enhancement === enhancement);
        if (!found) { hasAll = false; break; }
      }
      if (!hasAll) break;
    }
    
    if (hasAll) toReward.push(templateId);
  }
  
  // Batch reward all completed cards
  const rewarded: number[] = [];
  for (const templateId of toReward) {
    await updateCurrency(user.id, 100, 'gain');
    await prisma.userCardCompletion.create({
      data: { user_id: user.id, card_template_id: templateId }
    });
    rewarded.push(templateId);
  }
  
  res.json({ rewarded, amount: rewarded.length * 100 });
});

// Keep old endpoint for single checks
app.post('/api/user/check-completion', async (req, res) => {
  ...
  const existingReward = await prisma.userCardCompletion.findUnique({
    where: { user_id_card_template_id: { user_id: user.id, card_template_id: cardTemplateId } }
  });
  if (existingReward) return res.json({ alreadyRewarded: true });
  ...
  if (hasAll) {
    await updateCurrency(user.id, 100, 'gain');
    await prisma.userCardCompletion.create({
      data: { user_id: user.id, card_template_id: cardTemplateId }
    });
...
setInterval(() => {
  ...
  for (const [key, timestamp] of lastHeartbeat.entries()) {
    if (timestamp < oneHourAgo) lastHeartbeat.delete(key);
  }
}, 3600000);
...
app.post('/api/heartbeat', async (req, res) => {
  ...
  if (lastHeartbeat.has(key)) return res.json({ ok: true, duplicate: true });
  ...
  if (user) {
    ...
    // Only check achievements if play time increased by 5+ minutes
    const lastChecked = lastPlayTimeByUser.get(user.id) || 0;
    const currentStats = await prisma.userStats.findUnique({
      where: { user_id: user.id },
      select: { total_play_minutes: true }
    });
    const currentPlayTime = currentStats?.total_play_minutes || 0;
    
    if (currentPlayTime - lastChecked >= 5) {
      await checkAndUpdateAchievements(user.id, Condition.PLAY_TIME);
      lastPlayTimeByUser.set(user.id, currentPlayTime);
    }
  }
...
app.post('/api/cards/favorite', async (req, res) => {
  ...
  await prisma.userCards.updateMany({
    where: { user_id: user.id, card_template_id: cardId },
...
app.get('/api/cards/favorites', async (req, res) => {
  ...
  const favoriteCards = await prisma.userCards.findMany({
    where: { user_id: user.id, is_favourited: true },
    select: { card_template_id: true },
    distinct: ['card_template_id']
  });
  res.json({ favorites: Array.from(new Set(favoriteCards.map(c => c.card_template_id))) });
});
